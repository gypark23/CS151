...
;; Returns a new checker that has the clicked piece in the clicked-piece list and removed the clicked piece off from the piece list
...
;; takes a grid location and a game state and places theclicked-piece in that grid location
...
(: loc=? : Loc Loc -> Boolean)
;; checks if the two locations given are identical
(define (loc=? a b)
  (and (= (Loc-row a) (Loc-row b)) (= (Loc-col a) (Loc-col b))))

(: get-piece-or-none : Loc Checkers -> (U 'none Piece))
;; takes in a grid Loc and a Checkers and returns either aPiece if the game has a piece at that location, or ’none if no piece exists.
